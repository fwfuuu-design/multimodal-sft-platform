...
from .model_utils.moe import configure_moe
...
def patch_qwen3_omni_moe_thinker_text_sparse_moe_block():
    if is_transformers_version_greater_than("4.57.0") and not is_transformers_version_greater_than("4.58.0"):
        ...
        logger.warning_rank0(
            "Applying the Qwen3 Omni MoE compatibility patch required by this Transformers release."
        )
...
def patch_config(
    ...
    if init_kwargs["low_cpu_mem_usage"] and "device_map" not in init_kwargs and model_args.device_map:
...
def patch_model(
    ...
    try:
        model.add_model_tags(["multimodal-sft-platform"])
    except Exception:
