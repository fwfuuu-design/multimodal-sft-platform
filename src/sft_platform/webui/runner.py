...
from ..extras.constants import MULTIMODAL_SUPPORTED_MODELS, PEFT_METHODS, PLATFORM_CONFIG
...
class Runner:
    ...
    def _launch(self, data: dict["Component", Any], do_train: bool) -> Generator[dict["Component", Any], None, None]:
        ...
        if error:
            ...
            save_args(os.path.join(args["output_dir"], PLATFORM_CONFIG), self._build_config_dict(data))

            env = deepcopy(os.environ)
            env["SFT_PLATFORM_WEBUI_ENABLED"] = "1"
            env["SFT_PLATFORM_WORKDIR"] = args["output_dir"]
            # NOTE: DO NOT USE shell=True to avoid security risk
            webui_log_path = os.path.join(args["output_dir"], "webui_subprocess.log")
            webui_log = open(webui_log_path, "a", encoding="utf-8")
            self.trainer = Popen(
                ["sft-train", "train", save_cmd(args)],
                env=env,
                stdout=webui_log,
                stderr=webui_log,
                text=True,
    ...
    def load_args(self, lang: str, config_path: str):
        ...
        for elem_id, value in config_dict.items():
            try:
                ...
                continue  # ignore settings removed from the SFT-only product surface
    ...
    def check_output_dir(self, lang: str, model_name: str, finetuning_type: str, output_dir: str):
        ...
        if model_name and output_dir and os.path.isdir(get_save_dir(model_name, finetuning_type, output_dir)):
            ...
            config_dict = load_args(os.path.join(output_dir, PLATFORM_CONFIG))
            for elem_id, value in config_dict.items():
                try:
                    output_dict[self.manager.get_elem_by_id(elem_id)] = value
                except KeyError:
