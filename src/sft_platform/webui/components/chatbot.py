...
def create_chat_box(
    ...
    with gr.Column(visible=visible) as chat_box:
        ...
        if "buttons" in inspect.signature(gr.Chatbot.__init__).parameters:
            kwargs["buttons"] = ["copy"]
        if "resizable" in inspect.signature(gr.Chatbot.__init__).parameters:
            kwargs["resizable"] = True

        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", message=".*default value of 'allow_tags'.*", category=DeprecationWarning)
