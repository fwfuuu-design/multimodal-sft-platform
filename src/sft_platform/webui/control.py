...
from ..extras.constants import CHECKPOINT_NAMES, PEFT_METHODS, RUNNING_LOG, TRAINER_LOG
...
def change_stage(_training_stage: str = "Supervised Fine-Tuning") -> tuple[list[str], bool]:
    r"""Reset the dataset while keeping SFT packing disabled by default."""
    return [], False
...
def list_datasets(
    dataset_dir: str = None, training_stage: str = "Supervised Fine-Tuning"
) -> "gr.Dropdown":
    r"""List non-ranking datasets available to the SFT product."""
    if training_stage != "Supervised Fine-Tuning":
        return gr.Dropdown(choices=[])
    dataset_info = load_dataset_info(dataset_dir if dataset_dir is not None else DEFAULT_DATA_DIR)
    return gr.Dropdown(choices=[name for name, info in dataset_info.items() if not info.get("ranking", False)])
